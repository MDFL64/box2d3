...
async fn main() {
    ...
    let size = 50;
    {
        ...
        for x in 0..size {
            for y in 0..size {
                engine.add_body(BodyDef::new(Vec2::new((x as f32 - offset) * 2.0, y as f32 - offset + 20.0), vec!(
                    Polygon::new_box(1.0,1.0).into()
                ))).unwrap();
            }
        }
    }
    ...
    // ground
    engine.add_body(BodyDef::new(Vec2::new(0.0, 0.0), vec!(
        Polygon::new_box(100.0,50.0).offset(Vec2::new(0.0, -50.0)).into(),
    )).set_static()).unwrap();
    ...
    loop {
        ...
        if let Some(sim_time) = sim_time {
            draw_text(&format!("physics update: {:?}",sim_time), 10.0, 30.0, 32.0, WHITE);
        }
...
fn set_world_camera() {
    ...
    let camera =
        Camera2D::from_display_rect(Rect::new(-w/2.0, -h/2.0, w, h));
    set_camera(&camera);
}
